...
class OperationsHelper(BasePage):
    ...
    def enter_name(self, word):
        logging.info(f"Send '{word}' to element {TestSeacrhLocators.LOCATOR_NAME_FIELD[1]}")
        name_field = self.find_element(TestSeacrhLocators.LOCATOR_NAME_FIELD)
        name_field.clear()
        name_field.send_keys(word)

    def enter_email(self, word):
        logging.info(f"Send '{word}' to element {TestSeacrhLocators.LOCATOR_EMAIL_FIELD[1]}")
        email_field = self.find_element(TestSeacrhLocators.LOCATOR_EMAIL_FIELD)
        email_field.clear()
        email_field.send_keys(word)

    def enter_content_field(self, word):
        logging.info(f"Send '{word}' to element {TestSeacrhLocators.LOCATOR_CONTENT_FIELD[1]}")
    ...
    def click_contact_us_button(self):
        ...
        self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_US_BTN).click()
